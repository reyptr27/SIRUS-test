<div class="grid gap-10">
    ...
    <x-form-section submit="createApiToken">
        ...
        <x-slot name="form">
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-input id="name" type="text" class="form-input mt-1 block w-full" wire:model="createApiTokenForm.name" autofocus autocomplete="off" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-span for="permissions" value="{{ __('Permissions') }}" />

                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                <x-checkbox id="{{ $permission }}" name="{{ $permission }}" wire:model="createApiTokenForm.permissions" :value="$permission"/>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            ...
            <x-input id="plaintextToken" name="plaintextToken" x-ref="plaintextToken" type="text" readonly :value="$plainTextToken"
                class="form-input mt-4 bg-gray-100 px-4 py-2 rounded font-mono text-sm text-gray-500 w-full break-all"
    ...
    <x-dialog-modal wire:model.live="managingApiTokenPermissions">
        ...
        <x-slot name="content">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                    <label class="flex items-center">
                        <x-checkbox id="{{ $permission }}" name="{{ $permission }}" wire:model="updateApiTokenForm.permissions" :value="$permission"/>
                        <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $permission }}</span>
                    </label>
                @endforeach
            </div>
